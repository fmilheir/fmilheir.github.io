Office.onReady((info) => {
    if (info.host === Office.HostType.Outlook) {
        // Check if we're in a calendar item
        const item = Office.context.mailbox.item;
        const applyButton = document.getElementById('apply-template');
        
        if (item.itemType === Office.MailboxEnums.ItemType.Appointment) {
            // Enable button for calendar items
            applyButton.onclick = applyTemplate;
            applyButton.disabled = false;
        } else {
            // Disable button for non-calendar items
            applyButton.disabled = true;
            applyButton.classList.add('disabled');  // Add disabled styling
            applyButton.title = 'This template can only be used with calendar items';
        }
    }
});
